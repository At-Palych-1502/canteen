...
def allergies():
    ingredients = Ingredient.query.all()
    allergies = User.query.get(get_jwt_identity()).allergies
    return jsonify({"ingredients": [ingredient.to_dict() for ingredient in ingredients], "allergies": [ingr.to_dict() for ingr in allergies]})